...
export const deployViaChameleon = async (
  network: NetworkProvider,
  config: DeployConfig,
  nonces: bigint[],
  ...
  const chameleonContract = Cell.fromBase64(
    await compileFuncFileToBase64Boc(path, chameleonContractFilename),
  );
  const chameleonStateInit: StateInit = {
    code: chameleonContract,
    data: nonces
      .reduce(
        (prevCell: Builder, nextNoncePart: bigint) =>
          prevCell.storeInt(nextNoncePart, 32),
        beginCell(),
      )
      .endCell(),
  };
  ...
  const messageToChameleon = beginCell()
